...
def crypted_string(string):
    ...
    return crypted_string.hexdigest() #Conversion du mot de passe crypté en un nombre hexadecimal.
...
def accueil():
    if 'logged' in fl.session.keys():
        return fl.redirect(fl.url_for('profil'))
    else:
        ...
        if fl.request.method == 'GET':
            ...
            if request.form['email'] in user_mail_list:
                ...
                if request.form['password'] == pwd:
                    user_id = c.execute('SELECT user_id FROM Users WHERE email = ?', (request.form['email'],)).fetchone()[0]
                    conn.close()
...
def sign_up(): #page inscription
    ...
    #remplissage du formulaire et incrémentation de la base de données
        ...
        if fl.request.form['email'] in email_list : # Test pour savoir si l'email existe ou non
            ...
            return fl.redirect(fl.url_for('profil')) # Revenir dessus. Après stockage des données, envoie vers la vue profil
